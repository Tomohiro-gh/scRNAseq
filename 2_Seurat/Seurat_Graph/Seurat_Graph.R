...
## タイトルの文字を変える時：
## strip.text.xで制御
## https://github.com/satijalab/seurat/issues/5510#issuecomment-1018680899
...
fun.fp.batch <- function(object, genelist, samplename, row.num){
  ...
  ggsave(paste0("FP_",samplename,".png"), dpi=300, width=wid, height=hei)
}
...
################################################################################
##### Violin plot  　identを指定して，特定のcluster における遺伝子発現##########
...
#fun.vlp.split(seurat_obj, "ANGPT1", "Author_Annotation", "age", "")


##### Violin plot #############################################################
##### lapplyでかく方法 ########################################################
## refs: https://github.com/satijalab/seurat/issues/1425#issuecomment-485939697
plots <- VlnPlot(object = combined, 
                 features = c('Arg1', 'Tnf'), 
                 split.by = 'group', 
